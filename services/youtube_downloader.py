...
class YouTubeDownloader:
    ...
    def on_progress(self, stream, chunk, bytes_remaining):
        ...
        # specific progress bar implementation
        bar_length = 50
        filled_length = int(bar_length * bytes_downloaded // total_size)
        bar = '█' * filled_length + '-' * (bar_length - filled_length)
        
        sys.stdout.write(f'\r    Downloading: |{bar}| {percentage:.1f}%')
        sys.stdout.flush()
    ...
    def download(self, url):
        ...
        try:
            ...
            if progressive_streams:
                ...
                streams_list = list(progressive_streams)
                for i, s in enumerate(streams_list):
                    size_mb = s.filesize / (1024 * 1024)
                    print(f"  {i + 1}. {s.resolution} ({size_mb:.1f} MB)")
                
                while True:
                    try:
                        choice = input("\nSelect quality (enter number): ").strip()
                        idx = int(choice) - 1
                        if 0 <= idx < len(streams_list):
                            stream = streams_list[idx]
                            break
                        else:
            ...
            # If no progressive stream selected (or none found), try fallback
            if not stream:
                print("⚠️ No progressive stream selected or found, trying adaptive stream")
                stream = (
                    yt.streams
